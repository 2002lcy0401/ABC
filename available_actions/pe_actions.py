...
def section_add(exe_pth,adv_pth,content):
    ...
    print('section添加了',len(content),'个字节')
    print('Section添加完毕')
...
def header_modify(exe_path,adv_pth,new_data):
    ...
    if t<t1:
        ...
        print(f'new data length is less than indexes to perturb, only the {t} bytes will be inserted')
...
def code_cave(exe_path,adv_pth,new_data):
    ...
    with open(adv_pth, 'wb') as f:
        f.write(x_real_adv) 
    print('code cave添加了',t2*section_num,'个字节')
